...
package first_project.jreadcalendar_0_1;
...
/**
 * Job: jReadCalendar Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class jReadCalendar implements TalendJob {
	...
	public class ContextProperties extends PropertiesWithType {
		...
		public void synchronizeContext(){
			...
			if(dtDeb != null){
					...
					String value_dtDeb = "2024-01-01 00:00:00";
			...
			if(dtFin != null){
					...
					String value_dtFin = "2024-12-31 00:00:00";
			...
			if(dateDebExecution != null){
				
					String pattern_dateDebExecution = "yyyy-MM-dd HH:mm:ss";
					String value_dateDebExecution = "2024-08-08 11:41:55";
					String[] parts_dateDebExecution = value_dateDebExecution.split(";");
					if(parts_dateDebExecution.length > 1){
						pattern_dateDebExecution = parts_dateDebExecution[0];
						this.setProperty("dateDebExecution", pattern_dateDebExecution + ";" + FormatterUtils.format_Date(dateDebExecution, pattern_dateDebExecution));
					}else{
						this.setProperty("dateDebExecution", FormatterUtils.format_Date(dateDebExecution, pattern_dateDebExecution));
					}
...
public java.util.Date dateDebExecution;
public java.util.Date getDateDebExecution(){
	return this.dateDebExecution;
}
public Long nbJours;
public Long getNbJours(){
	...
	private final String jobName = "jReadCalendar";
	private final String projectName = "FIRST_PROJECT";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				jReadCalendar.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(jReadCalendar.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row_Implicit_Context_DatabaseStruct implements routines.system.IPersistableRow<row_Implicit_Context_DatabaseStruct> {
    final static byte[] commonByteArrayLock_FIRST_PROJECT_jReadCalendar = new byte[0];
    static byte[] commonByteArray_FIRST_PROJECT_jReadCalendar = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_FIRST_PROJECT_jReadCalendar.length) {
				if(length < 1024 && commonByteArray_FIRST_PROJECT_jReadCalendar.length == 0) {
   					commonByteArray_FIRST_PROJECT_jReadCalendar = new byte[1024];
				} else {
   					commonByteArray_FIRST_PROJECT_jReadCalendar = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_FIRST_PROJECT_jReadCalendar, 0, length);
			strReturn = new String(commonByteArray_FIRST_PROJECT_jReadCalendar, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_FIRST_PROJECT_jReadCalendar, 0, length);
			strReturn = new String(commonByteArray_FIRST_PROJECT_jReadCalendar, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_FIRST_PROJECT_jReadCalendar) {
 ...
 */
	...
	final String decryptedPassword_Implicit_Context_Database = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:bR3BuiKa6iKKwTg5Cq6AyEb1XhZzuOj/Lr8pna2b/8I=");
 ...
 */
  ...
  try{
        ...
        if(key_Implicit_Context_Context!=null && "dateDebExecution".equals(key_Implicit_Context_Context))
        {
                String context_dateDebExecution_value = context.getProperty("dateDebExecution");
                if(context_dateDebExecution_value==null)
                    context_dateDebExecution_value = "";
                int context_dateDebExecution_pos = context_dateDebExecution_value.indexOf(";");
                String context_dateDebExecution_pattern =  "yyyy-MM-dd HH:mm:ss";
                if(context_dateDebExecution_pos > -1){
                    context_dateDebExecution_pattern = context_dateDebExecution_value.substring(0, context_dateDebExecution_pos);
                }
                context.dateDebExecution=(java.util.Date)(new java.text.SimpleDateFormat(context_dateDebExecution_pattern).parse(value_Implicit_Context_Context));
        ...
        if(key_Implicit_Context_Context!=null && "nbJours".equals(key_Implicit_Context_Context))
        {
               ...
               context.nbJours=Long.parseLong(value_Implicit_Context_Context);
...
context.nbJours = TalendDate.diffDate(context.dtFin, context.dtDeb, "dd")+1;
context.dateDebExecution = TalendDate.getCurrentDate();

System.out.println("------------------------------------------------------------------");
System.out.println("Debut de la generation du calendrier : "+ TalendDate.formatDate("dd-MM-yyyy HH:mm:ss", context.dateDebExecution));
System.out.println("Nombre de jour a generer " + context.nbJours);
System.out.println("------------------------------------------------------------------");
...
public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
    ...
    static byte[] commonByteArray_FIRST_PROJECT_jReadCalendar = new byte[0];
			    ...
			    public Integer BL_DERNIER_JOUR_MOI;

				public Integer getBL_DERNIER_JOUR_MOI () {
					return this.BL_DERNIER_JOUR_MOI;
				}
				
			    public Integer BL_BISSEXTILE;

				public Integer getBL_BISSEXTILE () {
			    ...
			    public Integer BL_JOUR_FERIES;

				public Integer getBL_JOUR_FERIES () {
					return this.BL_JOUR_FERIES;
				}
        	...
        	try {
        		...
        		int length = 0;
					...
					this.LB_JOUR = readString(dis);
						...
						this.BL_DERNIER_JOUR_MOI = readInteger(dis);
					
						this.BL_BISSEXTILE = readInteger(dis);
					
						this.BL_WEEK_END = readInteger(dis);
					
						this.BL_JOUR_FERIES = readInteger(dis);
    ...
    public void writeData(ObjectOutputStream dos) {
        try {
					...
					// Integer
						...
						writeInteger(this.BL_DERNIER_JOUR_MOI,dos);
					
					// Integer
				
						writeInteger(this.BL_BISSEXTILE,dos);
					...
					// Integer
						...
						writeInteger(this.BL_JOUR_FERIES,dos);
    ...
    public String toString() {
		...
		sb.append(",BL_DERNIER_JOUR_MOI="+String.valueOf(BL_DERNIER_JOUR_MOI));
		sb.append(",BL_BISSEXTILE="+String.valueOf(BL_BISSEXTILE));
		sb.append(",BL_WEEK_END="+String.valueOf(BL_WEEK_END));
		sb.append(",BL_JOUR_FERIES="+String.valueOf(BL_JOUR_FERIES));
	    sb.append("]");
...
row3Struct row3 = new row3Struct();
	...
	///////////////////////
        ...
        util_tLogRow_1.addRow(new String[]{"ID_JOUR","DT_JOUR","NB_ANNEE","NB_SEMESTRE","NB_TRIMESTRE","NB_MOIS","NB_JOUR_ANNEE","NB_JOUR_MOIS","NB_JOUR_SEMAINE","NB_SEMAINE_ANNEE","LB_MOIS","LB_JOUR","BL_DERNIER_JOUR_MOI","BL_BISSEXTILE","BL_WEEK_END","BL_JOUR_FERIES",});        
 ...
 */
	...
	currentComponent="tFileOutputExcel_1";
		...
		String fileName_tFileOutputExcel_1=context.prFolder + context.filename;
    ...
    xlsxTool_tFileOutputExcel_1.setTruncateExceedingCharacters(false);
		...
		xlsxTool_tFileOutputExcel_1.addCellValue("BL_DERNIER_JOUR_MOI");
		
		xlsxTool_tFileOutputExcel_1.addCellValue("BL_BISSEXTILE");
		
		xlsxTool_tFileOutputExcel_1.addCellValue("BL_WEEK_END");
		
		xlsxTool_tFileOutputExcel_1.addCellValue("BL_JOUR_FERIES");
...
// # Output table : 'out1'
out1_tmp.DT_JOUR = TalendDate.addDate(context.dtDeb, row1.NUM_LIGNE, "dd") ;
out1 = out1_tmp;
// ###############################
...
// # Output table : 'out2'
out2_tmp.ID_JOUR = Integer.parseInt(TalendDate.formatDate("YYYYMMDD", out1.DT_JOUR)) ;
out2_tmp.DT_JOUR = out1.DT_JOUR ;
out2_tmp.NB_ANNEE = TalendDate.getPartOfDate("YEAR", out1.DT_JOUR) ;
out2_tmp.NB_SEMESTRE = TalendDate.getPartOfDate("MONTH", out1.DT_JOUR)<6?2:1 ;
out2_tmp.NB_TRIMESTRE = TalendDate.getPartOfDate("MONTH", out1.DT_JOUR)<3?1:
TalendDate.getPartOfDate("MONTH", out1.DT_JOUR)<6?2:
TalendDate.getPartOfDate("MONTH", out1.DT_JOUR)<9?3:4 ;
out2_tmp.NB_MOIS = TalendDate.getPartOfDate("MONTH", out1.DT_JOUR)+1 ;
out2_tmp.NB_JOUR_ANNEE = TalendDate.getPartOfDate("DAY_OF_YEAR", out1.DT_JOUR) ;
out2_tmp.NB_JOUR_MOIS = TalendDate.getPartOfDate("DAY_OF_MONTH", out1.DT_JOUR) ;
out2_tmp.NB_JOUR_SEMAINE = TalendDate.getPartOfDate("DAY_OF_WEEK", out1.DT_JOUR)-1==0?7:
TalendDate.getPartOfDate("DAY_OF_WEEK", out1.DT_JOUR)-1 ;
out2_tmp.NB_SEMAINE_ANNEE = TalendDate.getPartOfDate("WEEK_OF_YEAR", out1.DT_JOUR) ;
out2_tmp.LB_MOIS = TalendDate.formatDate("MMMM", out1.DT_JOUR).substring(0,1).toUpperCase() +
TalendDate.formatDate("MMMM", out1.DT_JOUR).substring(1, TalendDate.formatDate("MMMM", out1.DT_JOUR).length()) ;
out2_tmp.LB_JOUR = TalendDate.getPartOfDate("DAY_OF_WEEK", out1.DT_JOUR)==2?"Lundi":
TalendDate.getPartOfDate("DAY_OF_WEEK", out1.DT_JOUR)==3?"Mardi":
TalendDate.getPartOfDate("DAY_OF_WEEK", out1.DT_JOUR)==4?"Mercredi":
TalendDate.getPartOfDate("DAY_OF_WEEK", out1.DT_JOUR)==5?"Jeudi":
TalendDate.getPartOfDate("DAY_OF_WEEK", out1.DT_JOUR)==6?"Vendredi":
TalendDate.getPartOfDate("DAY_OF_WEEK", out1.DT_JOUR)==7?"Samedi":"Dimanche" ;
out2_tmp.BL_DERNIER_JOUR_MOI = TalendDate.compareDate(TalendDate.getLastDayOfMonth(out1.DT_JOUR), out1.DT_JOUR, "YYYY-MM-DD")==0?1:0 ;
out2_tmp.BL_BISSEXTILE = ((double)TalendDate.getPartOfDate("YEAR", out1.DT_JOUR)/4)==Math.floor(TalendDate.getPartOfDate("YEAR", out1.DT_JOUR)/4)?1:0 ;
out2_tmp.BL_WEEK_END = TalendDate.getPartOfDate("DAY_OF_WEEK", out1.DT_JOUR)==7 || TalendDate.getPartOfDate("DAY_OF_WEEK", out1.DT_JOUR)==1?1:0 ;
out2_tmp.BL_JOUR_FERIES = 0;
out2 = out2_tmp;
// ###############################
...
if(out2 != null) { 
	...
	row2.BL_DERNIER_JOUR_MOI = out2.BL_DERNIER_JOUR_MOI;								
	row2.BL_BISSEXTILE = out2.BL_BISSEXTILE;								
	row2.BL_WEEK_END = out2.BL_WEEK_END;								
	row2.BL_JOUR_FERIES = out2.BL_JOUR_FERIES;			
	row3 = new row3Struct();
	...
	row3.BL_DERNIER_JOUR_MOI = out2.BL_DERNIER_JOUR_MOI;								
	row3.BL_BISSEXTILE = out2.BL_BISSEXTILE;								
	row3.BL_WEEK_END = out2.BL_WEEK_END;								
	row3.BL_JOUR_FERIES = out2.BL_JOUR_FERIES;			
...
///////////////////////		
	    		...
	    		if(row2.BL_DERNIER_JOUR_MOI != null) { //              
                 row_tLogRow_1[12]=    						    
				                String.valueOf(row2.BL_DERNIER_JOUR_MOI)			
					          ;	
							
	    		} //			
    			   				
	    		if(row2.BL_BISSEXTILE != null) { //              
                 row_tLogRow_1[13]=    						    
				                String.valueOf(row2.BL_BISSEXTILE)			
					          ;	
	    		...
	    		if(row2.BL_JOUR_FERIES != null) { //              
                 row_tLogRow_1[15]=    						    
				                String.valueOf(row2.BL_JOUR_FERIES)			
					          ;	
 ...
 */
	...
	currentComponent="tFileOutputExcel_1";
				...
				xlsxTool_tFileOutputExcel_1.addRow();
	    				...
	    				if(row3.BL_DERNIER_JOUR_MOI != null) {
    				
							xlsxTool_tFileOutputExcel_1.addCellValue(Double.parseDouble(String.valueOf(row3.BL_DERNIER_JOUR_MOI)));
	    				} else {
	    					xlsxTool_tFileOutputExcel_1.addCellNullValue();
	    				}
					
									   				
	    				if(row3.BL_BISSEXTILE != null) {
    				
							xlsxTool_tFileOutputExcel_1.addCellValue(Double.parseDouble(String.valueOf(row3.BL_BISSEXTILE)));
	    				} else {
	    					xlsxTool_tFileOutputExcel_1.addCellNullValue();
	    				}
	    				...
	    				if(row3.BL_JOUR_FERIES != null) {
    				
							xlsxTool_tFileOutputExcel_1.addCellValue(Double.parseDouble(String.valueOf(row3.BL_JOUR_FERIES)));
	    				} else {
	    					xlsxTool_tFileOutputExcel_1.addCellNullValue();
	    				}
...
System.out.println("------------------------------------------------------------------");
System.out.println("Fin de la generation du calendrier : "+ TalendDate.formatDate("dd-MM-yyyy HH:mm:ss", context.dateDebExecution));
System.out.println("Temps d'execution " + TalendDate.diffDate(TalendDate.getCurrentDate(), context.dateDebExecution));
System.out.println("------------------------------------------------------------------");
 ...
 */
    ...
    public static void main(String[] args){
        final jReadCalendar jReadCalendarClass = new jReadCalendar();

        int exitCode = jReadCalendarClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = jReadCalendar.class.getClassLoader().getResourceAsStream("first_project/jreadcalendar_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = jReadCalendar.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
            ...
            class ContextProcessing {
                private void processContext_0() {
                        ...
                        context.setContextType("prFolder", "id_Directory");
                        ...
                        context.setContextType("dateDebExecution", "id_Date");
                        if(context.getStringValue("dateDebExecution") == null) {
                            context.dateDebExecution = null;
                        } else {
                            String context_dateDebExecution_value = context.getProperty("dateDebExecution");
                            try{
                                if (context_dateDebExecution_value == null){
                                ...
                                if(context_dateDebExecution_pos > -1){
                                    context_dateDebExecution_pattern = context_dateDebExecution_value.substring(0, context_dateDebExecution_pos);
                                    context_dateDebExecution_value = context_dateDebExecution_value.substring(context_dateDebExecution_pos + 1);
                                }

                                context.dateDebExecution=(java.util.Date)(new java.text.SimpleDateFormat(context_dateDebExecution_pattern).parse(context_dateDebExecution_value));

                            } catch(ParseException e) {
                                try { 
                                    long context_dateDebExecution_longValue = Long.valueOf(context_dateDebExecution_value);
                                    context.dateDebExecution = new java.util.Date(context_dateDebExecution_longValue);
                                } catch (NumberFormatException cantParseToLongException) {
                                        System.err.println(String.format("Null value will be used for context parameter %s: %s", "dateDebExecution", "Can't parse date string: " + e.getMessage() + " and long: " + cantParseToLongException.getMessage()));
                                    context.dateDebExecution=null;
                                }
                            }
                        }
                        context.setContextType("nbJours", "id_Long");
                        if(context.getStringValue("nbJours") == null) {
                            context.nbJours = null;
                        } else {
                            try{
                                context.nbJours=routines.system.ParserUtils.parseTo_Long (context.getProperty("nbJours"));
        ...
        if (parentContextMap != null && !parentContextMap.isEmpty()) {if (parentContextMap.containsKey("dtDeb")) {
            ...
            }if (parentContextMap.containsKey("dateDebExecution")) {
                context.dateDebExecution = (java.util.Date) parentContextMap.get("dateDebExecution");
            }if (parentContextMap.containsKey("nbJours")) {
                context.nbJours = (Long) parentContextMap.get("nbJours");
            }
        }
...
e_tPostjob_1.printStackTrace();
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : jReadCalendar");
        }
...
/************************************************************************************************
 *     162094 characters generated by Talend Open Studio for Data Integration 
 *     on the 8 août 2024 à 12:53:35 GMT
 ************************************************************************************************/